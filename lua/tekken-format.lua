...
---@param line string
---@return string | nil
local function find_character_in_line(line)
  local last_match = nil
  local prev_cleaned = nil
  for word in string.gmatch(line, '([^%s]+)') do
    ...
    -- check two-word combination first (e.g., "Armor King" → "armor-king", "Devil Jin" → "devil-jin")
    if prev_cleaned then
      local combined = prev_cleaned .. '-' .. cleaned
      if is_valid_character(combined) then
        last_match = combined
      end
    end
...
local function get_closest_character_name()
  ...
  -- First pass: prefer character names found in heading lines
  for i = line_nr, 1, -1 do
    local line = vim.api.nvim_buf_get_lines(buf, i - 1, i, false)[1]
    if line:match('^#') then
      local character = find_character_in_line(line)
      if character then
        return character
      end
    end
